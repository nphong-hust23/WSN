...
SHT3x_status SHT3x_read_bytes(SHT3x_config* sht3x,uint8_t address, uint16_t length, uint32_t timeout) {
    ...
    for(int i = 0; i < 2; i++){
        ...
        if(SHT3x_CheckCrc8(p,2) != p[2]) return SHT3x_CRC_ERROR;
    }
    sht3x->temp = (uint16_t)buffer[0] << 8 | buffer[1];
    sht3x->humid = (uint16_t)buffer[3] << 8 | buffer[4];
    return SHT3X_OK;
}
