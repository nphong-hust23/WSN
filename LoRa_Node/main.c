...
void mPrint(const char *format,...){
	...
	va_end(args);
	HAL_UART_Transmit(&huart1,(uint8_t *)buff, sizeof(buff), 1000);
}
...
uint32_t LoRa_Platform_GetTickMs(void) {
    return HAL_GetTick();   // đã là millisecond
}
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  extern void Test_Node_Init(void);
  extern void Test_Node_Run(void);
